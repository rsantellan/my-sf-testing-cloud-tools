<?php
class mymagick_decorations
{
    public function roundCorners(myMagick $p,$i = 15)
    {
    ...
    public function dropShadow(myMagick $p,$color = '#000', $offset = 4, $transparency = 60, $top = 4, $left=4)
    {
    	$top = $top > 0 ? '+' . $top : $top;
    ...
    public function glow(myMagick $p, $color='#827f00',$offset = 10, $transparency=60)
    {
     ...
     */
    public function fakePolaroid(myMagick $p,$rotate = 6 , $borderColor = "#fff", $background ="none")
    {
        $cmd = $p->getBinary('convert');
        $cmd .= ' "' . $p->getSource().'"'  ;
     ...
     *
     * @param $format myMagickTextObject - text format for image label
     * @param $rotation Int - The imahe will be rotatex x degrees
     * @param $borderColor - Polaroid border (ussuay white)
     * @param $shaddowColor - drop shaddow color
     * @param $background - Image background color (use for jpegs or images that do not support transparency or you will end up with a black background)
     */
    public function polaroid(myMagick $p, $format = null, $rotation= 6, $borderColor="snow", $shaddowColor = "black", $background="none")
    {
        if (get_class($format) == 'myMagickTextObject' ){
            //
        }else{
            $tmp = new myMagickTextObject();
            $tmp->text($format);
            $format = $tmp ;
        }
    ...
    public function border(myMagick $p,$borderColor = "#000", $borderSize ="1"){
        $cmd = $p->getBinary('convert');
        $cmd .= ' "' . $p->getSource() .'"';

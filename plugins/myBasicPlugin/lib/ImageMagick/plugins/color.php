...
class myMagick_color {

    public function darken(myMagick $p ,$alphaValue = 50)
    {
     ...
     */
    public function brighten(myMagick $p, $alphaValue = 50){
        $percent = 100 - (int) $alphaValue;
...
//    }
    ...
    public function toGreyScale(myMagick $p, $enhance=2){
        $cmd   = $p->getBinary('convert');
        $cmd .= ' -modulate 100,0 ' ;
     ...
     */
    public function invertColors(myMagick $p){
        $cmd  = $p->getBinary('convert');
        $cmd .= ' "' . $p->getSource() .'"';
        $cmd .= ' -negate ';
        $cmd .= ' "' . $p->getDestination() .'"';
    ...
    public function sepia(myMagick $p, $tone = 90){
        $cmd   = $p->getBinary('convert');
        $cmd .= ' -sepia-tone '. $tone .'% ' ;
        $cmd .= ' -modulate 100,50 ' ;
        $cmd .= ' -normalize ' ;
    ...
    public function autoLevels(myMagick $p){
        $cmd  = $p->getBinary('convert');
        $cmd .= ' -normalize ' ;
        $cmd .= ' -background "none" "' . $p->getSource().'"'  ;
        $cmd .= ' "' . $p->getDestination() .'"' ;

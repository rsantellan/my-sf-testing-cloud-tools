...
define('LOCAL_PATH', sfConfig::get('app_ImageFileHandler_path', sfConfig::get('sf_upload_dir')));
...
class myFileHandler {
    ...
    public static function upload($FILES, $path = LOCAL_PATH, $keep_real_name = false){
        ...
        sfContext::getInstance()->getLogger()->info('>>>>>>> save path >>>>>>>>>>' . $save_path);
        $upload_name = "upload";
...
*/        
        ...
        sfContext::getInstance()->getLogger()->info('>>>>>>> save file >>>>>>>>>>' . $save_path. $file_name);
            ...
            if ($dh = opendir($path)) {
                while (($file = readdir($dh)) !== false) {
                    ...
                    if (is_dir($path . $file) AND $file != '.' AND $file != '..' AND $file != '.svn'){
                       ...
                       self::getList($path . $file . DIRECTORY_SEPARATOR);
	...
	public static function checkDirectory($path){
        ...
        $new_path = str_replace($rootDir, "", $path);
        //sfContext::getInstance()->getLogger()->info('>>>>>>> new path >>>>>>>>>>' . $new_path);
        $folders = $pieces = explode(DIRECTORY_SEPARATOR, $new_path);
		$smallPath = $rootDir;
        
        //sfContext::getInstance()->getLogger()->info('>>>>>>> smallPath >>>>>>>>>>' . $smallPath);

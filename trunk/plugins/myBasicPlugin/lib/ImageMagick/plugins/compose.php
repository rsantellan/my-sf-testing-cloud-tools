<?php
class myMagick_compose
{
    /**
     * Add's an watermark to an image
     *
     * @param $watermarkImage String - Image path
     * @param $gravity myMagickGravity - The placement of the watermark
     * @param $transparency Integer - 1 to 100 the tranparency of the watermark (100 = opaque)
     */
    public function watermark(myMagick $p, $watermarkImage, $gravity = 'center', $transparency = 50){
     ...
     */
    public function tile(myMagick $p,  Array $paths = null, $tileWidth = '', $tileHeight=1){
        if( is_null($paths) ) {
            $paths = $p->getHistory(myMagickHistory::returnArray);
        }
     ...
     */
    public function acquireFrame(myMagick $p, $file, $frames=0)
    {
